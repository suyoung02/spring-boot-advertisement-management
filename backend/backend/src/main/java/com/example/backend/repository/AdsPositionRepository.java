...
public interface AdsPositionRepository extends JpaRepository<AdsPosition, Integer> {
    @Query(value = "SELECT position,location,adsFrom,status FROM AdsPosition position JOIN LocationType location ON location.title = position.location_type JOIN AdsForm adsFrom ON adsFrom.title = position.ads_form JOIN PlanningStatus status ON status.title=position.planning_status")

...
import { AdsType, LocationType } from '@/types/ads';
...
const ManagePositionStatus = () => {
  ...
  const { mutate: update } = useMutation({
    mutationFn: (data: UpdatePlanningStatusRequest) =>
      updatePlanningStatus(data),
    onSuccess: () => {
      handleCancel();
      refetch();
    },
  ...
  const { mutate: addAds } = useMutation({
    mutationFn: (data: LocationType) => addPlanningStatus(data),
    onSuccess: () => {
      handleCancel(true);
      refetch();
    },
  ...
  const { mutate: deleteAds } = useMutation({
    mutationFn: (data: string) => deletePlanningStatus(data),
    onSuccess: () => {
      handleCancel();
      refetch();
    },
  ...
  const handleUpdate = async () => {
    ...
    update({
      ...
      planningStatus: { ...fields, icon: icon || '' },
    });
  };
  ...
  return (
    <div className="p-8 pt-6">
      <div className="font-bold mb-3 text-xl">Quản lý trạng thái vị trí</div>
...
export default ManagePositionStatus;
